...
router.get('/bac/requests', authenticateToken, async (req: Request, res: Response) => {
  try {
    ...
    if (status === 'completed') {
      // For completed, only filter by event status = completed (no date restriction)
      query = {
        location,
        status: 'completed'
      };
    } else {
      // For pending/approved/rejected, filter by active events
      query = {
        location,
    ...
    if (status === 'pending') {
      ...
      // For rejected, show both active events and completed events that were rejected by BAC
      query = {
        location,
        $or: [
          { status: { $in: ['submitted', 'approved'] }, endDate: { $gte: today }, bacApprovalStatus: 'rejected' },
          { status: 'completed', bacApprovalStatus: 'rejected' }
        ]
      };
    }
...
router.patch('/:id/status', authenticateToken, async (req: Request, res: Response) => {
  try {
    ...
    if (!isAdmin) {
      ...
      // Requestor cancellation rules:
      // - allow cancelling approved/submitted/draft events
      // - disallow cancelling completed/cancelled events
      ...
      if (endDate && !Number.isNaN(endDate.getTime())) {
        ...
        if (endDate < today) {
          return res.status(400).json({
            success: false,
            message: 'Past events can no longer be cancelled.'
          });
        }
      }
    }
